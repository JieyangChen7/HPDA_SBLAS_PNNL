...
/* CPU version spmspv for correctness verification */
void csr_spmspv_cpu(int m, int n, long long nnz, 
		   double * alpha,
                   double * cooVal, long long * csrRowPtr, int * cooColIndex,
                   double * x, 
	           double * beta,
                   double * y) { 
  for (int i = 0; i < m; i++) {
...
int main(int argc, char *argv[]) {
	...
	*/
	...
	for (int i = 0; i < repeat_test; i++) {
		...
		curr_time = get_time();
		ret2 = spMspV_mgpu_v1(m, n, nnz, &ALPHA,
					 cooVal, csrRowPtr, cooColIndex, 
					 x, &BETA,
					 y2,
					 ngpu,
		...
		curr_time = get_time();
		ret3 = spMspV_mgpu_v2(m, n, nnz, &ALPHA,
					 cooVal, csrRowPtr, cooColIndex, 
					 x, &BETA,
					 y3,
					 //ngpu,
		...
		for(int i = 0; i < m; i++) {
		  ...
		  if (abs(y_verify[i] - y1[i]) > 1e-6) {
                    //cout << y_verify[i] << " - " << y2[i] << endl;
                    correct1 = false;
                  }
                  if (abs(y_verify[i] - y2[i]) > 1e-6) {
	            //cout << y1_verify[i] << " - " << y2[i] << endl;
		    correct2 = false;
	          }
		  if (abs(y_verify[i] - y3[i]) > 1e6) {
	            //cout << y1_verify[i] << " - " << y3[i] << endl;
		    correct3 = false;
		  }
		}
