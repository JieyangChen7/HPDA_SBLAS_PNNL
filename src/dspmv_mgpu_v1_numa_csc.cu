...
spmv_ret spMV_mgpu_v1_numa_csc(int m, int n, long long nnz, double * alpha,
  ...
  #pragma omp parallel default (shared) reduction(max:numa_part_time)
  {
    ...
    if (numaContext.representiveThreads[dev_id]) {
      ...
      print_vec_gpu(pcscNuma[numa_id].dval, pcscNuma[numa_id].nnz, "cscVal"+to_string(dev_id));
      print_vec_gpu(pcscNuma[numa_id].dcolPtr, pcscNuma[numa_id].n + 1, "colPtr"+to_string(dev_id));
      print_vec_gpu(pcscNuma[numa_id].drowIdx, pcscNuma[numa_id].nnz, "rowIdx"+to_string(dev_id));
      print_vec_gpu(pcscNuma[numa_id].dx, pcscNuma[numa_id].n, "x"+to_string(dev_id));
      print_vec_gpu(pcscNuma[numa_id].dy, pcscNuma[numa_id].m, "y_before"+to_string(dev_id));
